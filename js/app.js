...
(() => {
  ...
  const BUILTIN_COLLECTIONS = [
    ...
    { id: 'athletic',         file: './data/collections/athletic.json' },
    { id: 'boat',             file: './data/collections/boat.json' },
...
`
          + `別の端末の記録も、そのまま足されます。
`
          + `同じ記録が両方にあるときは、新しく直した方を残します。
`
          + `通った道・通った市区町村・集めるリストの印は、両方を合わせます（消えません）。
...
`
        ...
        try {
          ...
          state.collections = null;           // 合わせた自分のリスト・印を数え直す
          await refreshVisited();
        ...
        alert('読み込みました。\n\n'
          + '　記録 ' + r.visits + ' 件'
          + (r.visits ? '（新しく足した ' + r.added + '・新しい内容にした ' + r.replaced
            + '・この端末の方が新しかった ' + r.kept + '）' : '') + '\n'
          + '　場所 ' + r.spots + ' 件\n'
